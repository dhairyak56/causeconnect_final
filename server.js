...
// Routes
app.get('/', (req, res) => {
    db.query('SELECT * FROM events WHERE approved = TRUE', (err, results) => {
        if (err) {
...
app.post('/events', ensureAuthenticated, [
    ...
    db.query('INSERT INTO events (name, organization, location, time, description, date, image, user_id, approved) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', 
    [name, organization, location, time, description, date, image, req.user.id, false], (err, results) => {
        if (err) {
            console.error('Database Error:', err);
...
app.get('/admin', ensureAdmin, (req, res) => {
    db.query('SELECT * FROM users', (err, users) => {
        ...
        db.query('SELECT * FROM events WHERE approved = FALSE', (err, pendingEvents) => {
            if (err) {
                console.error('Database Error:', err);
                req.flash('error', 'There was an error retrieving the pending events.');
                return res.redirect('/dashboard');
            }
            res.render('admin', { users: users, pendingEvents: pendingEvents, user: req.user, messages: req.flash() });
        });
    });
});
...
app.post('/approve_event', ensureAdmin, [
    ...
    const { eventId } = req.body;
    db.query('UPDATE events SET approved = TRUE WHERE id = ?', [eventId], (err, results) => {
        if (err) {
            console.error('Database Error:', err);
